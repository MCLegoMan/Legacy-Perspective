...
public class HoldPerspective {
	...
	public static void tick(MinecraftClient client) {
		for (int i = 0; i < 2; i++) {
			if (Keybindings.HOLD_PERSPECTIVE_FRONT.isPressed()) {
				if (!Keybindings.HOLD_PERSPECTIVE_BACK.isPressed() && !BACK_LOCK) {
					if (!FRONT_LOCK) {
			...
			if (!Keybindings.HOLD_PERSPECTIVE_FRONT.isPressed() && FRONT_LOCK) {
				FRONT_LOCK = false;
				client.options.perspective = FRONT_PREV;
			}
			if (Keybindings.HOLD_PERSPECTIVE_BACK.isPressed()) {
				if (!Keybindings.HOLD_PERSPECTIVE_FRONT.isPressed() && !FRONT_LOCK) {
					if (!BACK_LOCK) {
